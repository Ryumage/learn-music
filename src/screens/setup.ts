...
function chips(def: SettingDef, value: ModuleSettings[string] | undefined): string {
  if (def.type === 'toggle') {
    const on = value === true;
  ...
  return (def.options ?? [])
    .map((o) => {
      ...
      return `<button type="button" class="chip${on ? ' is-on' : ''}" aria-pressed="${on}" data-action="setting" data-setting="${esc(def.id)}" data-value="${esc(o.value)}">${esc(o.label)}</button>`;
    })
    .join('');
}
...
export function renderSetup(mod: ModuleDef, settings: ModuleSettings, store: Store): string {
  ...
  const rows = mod.settings
    .map(
      (d) => `<div class="setting">
        ...
        <div class="chips" role="group" aria-label="${esc(d.label)}">${chips(d, settings[d.id])}</div>
      </div>`,
    )
    .join('');
  return `
    <main class="app">
      ...
      <div class="actions">
        <button type="button" class="btn btn-primary" data-action="start" data-testid="start">Los geht’s · ${count} Fragen</button>
